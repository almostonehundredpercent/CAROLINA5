...
class BookingController extends Controller
{
    ...
    private function blockedRanges(Room $room)
    {
        ...
        if (in_array($room->operational_status, ['cleaning', 'maintenance'], true) && $room->operational_until?->isFuture()) {
            $ranges->push(['start' => now()->toDateString(), 'end' => $room->operational_until->copy()->addDay()->toDateString()]);
        }
        return $ranges;
    ...
    public function store(Request $request, Room $room)
    {
        ...
        if ($request->input('checkout_type') === 'guest') $rules += ['guest_name' => 'required|string|max:255', 'guest_email' => 'required|email:rfc,dns|max:255', 'guest_phone' => ['required', 'regex:/^(?:\\+63|63|0)9\\d{9}$/'], 'terms_accepted' => 'accepted'];
        ...
        $booking = DB::transaction(function () use ($room, $data, $guestData, $request, $nights, $total, $selectedAddOns, $checkInAt, $checkOutAt) {
            $lockedRoom = Room::whereKey($room->id)->lockForUpdate()->firstOrFail();
            Room::releaseExpiredOperationalBlocks();
            Booking::releaseExpiredHolds();
            $lockedRoom->refresh();
            abort_if(! $lockedRoom->is_active || $lockedRoom->operational_status !== 'available', 422, 'This room is temporarily unavailable.');
            $taken = $lockedRoom->bookings()->blocking()->where(function ($query) use ($checkInAt, $checkOutAt) {
                $query->whereNotNull('check_in_at')->where('check_in_at', '<', $checkOutAt)->where('check_out_at', '>', $checkInAt)
                    ->orWhere(fn ($legacy) => $legacy->whereNull('check_in_at')->where('check_in', '<', $checkOutAt->toDateString())->where('check_out', '>', $checkInAt->toDateString()));
            })->exists();
            abort_if($taken, 422, 'Those dates or hours are no longer available for this room.');
            return Booking::create($data + $guestData + ['user_id' => $request->user()?->id, 'room_id' => $lockedRoom->id, 'nights' => $nights, 'total_amount' => $total, 'add_ons' => $selectedAddOns, 'hold_expires_at' => now()->addMinutes(15), 'payment_method' => 'cash', 'status' => 'pending']);
        });
        try {
        ...
        return redirect()->route('bookings.receipt', $booking)->with('success', 'Reservation request recorded. Staff has 15 minutes to review it before the temporary room hold expires.');
    }
