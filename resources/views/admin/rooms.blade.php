        ...
        <section class="room-card-grid" aria-label="Room operations">
            @forelse($rooms as $room)
                ...
                <article class="room-card {{ $room->display_status }}">
                    ...
                    <details class="room-manage"><summary>Manage cleaning or maintenance</summary><form method="POST" action="{{ route('admin.rooms.status',$room) }}" class="room-operation-form">@csrf @method('PATCH')<div class="operation-form-grid"><div class="operation-field full"><label for="status-{{ $room->id }}">Operation type</label><select id="status-{{ $room->id }}" name="operational_status"><option value="cleaning">Cleaning</option><option value="maintenance">Maintenance</option><option value="available">Clear an active block now</option></select></div><div class="operation-field"><label for="start-{{ $room->id }}">Starts</label><input id="start-{{ $room->id }}" name="operational_starts_at" class="operation-start" type="datetime-local" min="{{ now()->format('Y-m-d\TH:i') }}" value="{{ now()->format('Y-m-d\TH:i') }}"></div><div class="operation-field"><label for="end-{{ $room->id }}">Ends</label><input id="end-{{ $room->id }}" name="operational_until" class="operation-end" type="datetime-local" min="{{ now()->addMinute()->format('Y-m-d\TH:i') }}"></div></div><div class="schedule-timer" aria-live="polite"><div><div class="schedule-timer-label">Scheduled length</div><div class="schedule-timer-value">Choose a duration</div></div><div class="duration-presets" aria-label="Quick duration"><button type="button" class="duration-button" data-minutes="60">1 hr</button><button type="button" class="duration-button" data-minutes="120">2 hrs</button><button type="button" class="duration-button" data-minutes="240">4 hrs</button><button type="button" class="duration-button" data-minutes="480">8 hrs</button><button type="button" class="duration-button" data-minutes="1440">24 hrs</button></div></div><div class="operation-form-grid"><div class="operation-field full"><label for="note-{{ $room->id }}">Staff note (optional)</label><input id="note-{{ $room->id }}" name="notes" maxlength="255" placeholder="For example: deep clean after checkout"></div></div><p class="operation-help">Choose a quick duration to set the end automatically, or select any start and end time yourself.</p><div class="operation-footer">@if(auth()->user()->isAdmin())<label class="override-note"><input type="checkbox" name="force_override" value="1"> Manager override if this overlaps a confirmed stay.</label>@else<span class="override-note">Confirmed stays cannot be overridden by your role.</span>@endif<button class="operation-save" type="submit">Save task</button></div></form></details>
                </article>
            @empty
...
<script>
    (() => {
        ...
        document.querySelectorAll('.room-operation-form').forEach((form) => {
            const start = form.querySelector('.operation-start');
            const end = form.querySelector('.operation-end');
            ...
            const refresh = () => {
                ...
                if (!start.value || !end.value || !Number.isFinite(minutes) || minutes <= 0) {
            ...
            buttons.forEach((button) => button.addEventListener('click', () => {
                ...
                if (!start.value || Number.isNaN(startDate.getTime())) return;
                startDate.setMinutes(startDate.getMinutes() + Number(button.dataset.minutes));
                end.value = toLocalInput(startDate);
                end.min = toLocalInput(new Date(new Date(start.value).getTime() + 60000));
                refresh();
            }));
            start.addEventListener('change', () => {
                if (start.value) end.min = toLocalInput(new Date(new Date(start.value).getTime() + 60000));
                refresh();
            });
