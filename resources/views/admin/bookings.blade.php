...
<div class="admin-shell">
    ...
    <main class="admin-main">
        ...
        <section class="panel bookings-panel" style="margin-top:20px"><div class="panel-heading"><div><p class="admin-kicker">ALL RESERVATIONS</p><h2>Individual bookings</h2></div><span class="booking-count">{{ $individualBookings->total() }} total</span></div><div class="admin-table-wrap"><table><thead><tr><th>Guest</th><th>Reference</th><th>Room</th><th>Stay dates</th><th>Payment</th><th>Total</th><th>Status</th></tr></thead><tbody>@forelse($individualBookings as $booking)<tr><td><b>{{ $booking->guest_name ?? $booking->user?->name ?? 'Guest' }}</b><small>{{ $booking->guest_email ?? $booking->user?->email ?? 'No email provided' }}</small><small>{{ $booking->guest_phone ?? 'No phone number provided' }}</small></td><td>{{ $booking->reference }}</td><td>{{ $booking->room?->name ?? 'Room removed' }}</td><td>{{ $booking->check_in->format('M j, Y') }}<small>to {{ $booking->check_out->format('M j, Y') }}</small></td><td>{{ $booking->payment_method === 'cash' ? 'Pay at property' : 'GCash' }}</td><td>₱{{ number_format($booking->total_amount) }}</td><td><span class="mini-status {{ $booking->status }}">{{ ucfirst($booking->status) }}</span></td></tr>@empty<tr><td colspan="7" class="no-bookings">No bookings have been created yet.</td></tr>@endforelse</tbody></table></div>@if($individualBookings->hasPages())<div style="margin-top:16px">{{ $individualBookings->links() }}</div>@endif</section>
        <section class="panel bookings-panel" style="margin-top:20px"><div class="panel-heading"><div><p class="admin-kicker">BOOKING CONTROL</p><h2>Confirm, cancel, or reserve</h2></div></div><div class="admin-table-wrap"><table><thead><tr><th>Guest</th><th>Room</th><th>Stay</th><th>Current status</th><th>Actions</th></tr></thead><tbody>@forelse($individualBookings as $booking)<tr><td><b>{{ $booking->guest_name ?? $booking->user?->name ?? 'Guest' }}</b><small>{{ $booking->reference }}</small></td><td>{{ $booking->room?->name ?? 'Room removed' }}</td><td>{{ $booking->check_in->format('M j') }} – {{ $booking->check_out->format('M j, Y') }}</td><td><span class="mini-status {{ $booking->status }}">{{ ucfirst($booking->status) }}</span></td><td style="display:flex;gap:8px;align-items:center">@if($booking->status === 'pending')<form method="POST" action="{{ route('admin.bookings.update', $booking) }}">@csrf @method('PATCH')<input type="hidden" name="status" value="confirmed"><button class="status-select confirmed">Confirm</button></form>@endif @if($booking->status !== 'cancelled')<form method="POST" action="{{ route('admin.bookings.update', $booking) }}">@csrf @method('PATCH')<input type="hidden" name="status" value="cancelled"><button class="status-select cancelled">Cancel</button></form>@else<form method="POST" action="{{ route('admin.bookings.update', $booking) }}">@csrf @method('PATCH')<input type="hidden" name="status" value="pending"><button class="status-select">Reserve again</button></form>@endif</td></tr>@empty<tr><td colspan="5" class="no-bookings">No bookings available to manage.</td></tr>@endforelse</tbody></table></div></section>
    </main>
</div>
</body>
</html>
