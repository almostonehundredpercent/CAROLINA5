...
<section class="section compact">
    <form class="filter-bar rooms-search" method="GET">
        ...
        <label class="rooms-time-field" id="rooms-time-field" hidden><span>Check-in time</span><input name="check_in_time" type="time" value="{{ $filters['check_in_time'] ?? '12:00' }}"></label>
...
document.addEventListener('DOMContentLoaded', () => {
    const calendar=document.getElementById('rooms-search-calendar'),days=document.getElementById('rooms-calendar-days'),month=document.getElementById('rooms-calendar-month'),note=document.getElementById('rooms-calendar-note'),checkIn=document.getElementById('rooms-check-in'),trigger=document.getElementById('rooms-check-in-trigger'),stay=document.getElementById('rooms-stay'),timeField=document.getElementById('rooms-time-field');
    ...
    const sync=()=>{trigger.querySelector('b').textContent=pretty(checkIn.value);timeField.hidden=stay.value==='day';note.textContent=stay.value==='day'?'Choose your check-in date. Your one-day stay ends automatically the next day.':'Choose your check-in date and arrival time. Your hourly check-out is calculated automatically.'};
