...
<style>.clean-select{position:relative}.clean-select-trigger,.selector-display{width:100%;min-height:42px;padding:10px 12px;border:1px solid var(--line);border-radius:6px;background:#fff;color:var(--ink);font:600 .86rem 'DM Sans';text-align:left}.clean-select-trigger{cursor:pointer}.clean-select-trigger::after{content:'⌄';float:right;color:var(--deep);font-size:1rem}.clean-select-menu{position:absolute;z-index:6;top:calc(100% + 7px);left:0;width:100%;padding:6px;border:1px solid var(--line);border-radius:8px;background:#fff;box-shadow:0 14px 30px rgba(41,29,18,.16)}.clean-select-menu[hidden]{display:none}.clean-select-option{display:block;width:100%;padding:10px 11px;border:0;border-radius:5px;background:#fff;color:var(--ink);font:600 .82rem 'DM Sans';text-align:left;cursor:pointer}.clean-select-option:hover,.clean-select-option.selected{background:#fff0d8;color:var(--deep)}</style>
    ...
    <div class="booking-form">
        ...
        @if(auth()->check() || $isGuest)
            <form method="POST" action="{{ route('bookings.store', $room) }}">
                ...
                @endif
                @if($room->guests === 1)<input type="hidden" name="guests" value="1">@else<label class="clean-select">Guests<input name="guests" type="hidden" value="{{ old('guests', 1) }}"><button type="button" class="clean-select-trigger" data-select-trigger>1 guest</button><section class="clean-select-menu" hidden>@for($i = 1; $i <= $room->guests; $i++)<button type="button" class="clean-select-option" data-select-value="{{ $i }}">{{ $i }} guest{{ $i > 1 ? 's' : '' }}</button>@endfor</section></label>@endif

                <fieldset class="booking-add-ons"><legend>Add to your stay</legend><small>Optional extras are added to your total.</small>@foreach($addOns as $key => $addOn)<label><input type="checkbox" name="add_ons[]" value="{{ $key }}" @checked(in_array($key, old('add_ons', [])))><span>{{ $addOn['label'] }}</span><b>₱{{ number_format($addOn['price']) }}</b></label>@endforeach</fieldset>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (hourlyHours) {
        const total = document.getElementById('hourly-total'); const packageRate = {{ $room->rental_hours ? $room->price_per_night : 'null' }}; const packageHours = {{ $room->rental_hours ?: 'null' }}; const rate = {{ $room->price_per_night / 24 }}; const addOnPrices = @json(collect($addOns)->mapWithKeys(fn ($item, $key) => [$key => $item['price']]));
        ...
        const refresh = () => { const hours = Number(hourlyHours.value); const multiplier = ({48:2,72:3,96:4,120:5,168:7})[hours] ?? 1, extras = [...document.querySelectorAll('input[name="add_ons[]"]:checked')].reduce((sum, input) => sum + Number(addOnPrices[input.value] ?? 0), 0); total.textContent = `Estimated total: ₱${((packageRate ? packageRate * multiplier : rate * hours) + extras).toLocaleString('en-PH', { minimumFractionDigits: 2 })}`; };
        hourlyHours.addEventListener('change', refresh); document.querySelectorAll('input[name="add_ons[]"]').forEach(input => input.addEventListener('change', refresh)); refresh();
